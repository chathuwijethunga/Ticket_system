...
<body class="bg-gray-100 p-8">
    <div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Edit Ticket (ID: {{ $ticket->id }})</h1>
            <a href="{{ route('tickets.index') }}" class="text-blue-500 hover:underline">Back to Tickets</a>
        </div>

        <form action="{{ route('tickets.update', $ticket->id) }}" method="POST">
            @csrf {{-- CSRF protection --}}
            @method('PUT') {{-- Method Spoofing for PUT/PATCH --}}
            ...
            <div class="mb-4">
                ...
                <textarea name="issue_description" id="issue_description" rows="5"
                          ...
                          required>{{ old('issue_description', $ticket->issue_description) }}</textarea>
